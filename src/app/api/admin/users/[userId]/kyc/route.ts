...
import { updateCustomerKycInputSchema } from "@/application/admin";
...
export async function PATCH(request: NextRequest, routeContext: RouteContext) {
  ...
  try {
    ...
    const input = await parseJson(request, updateCustomerKycInputSchema);
    const service = await createAdminCustomerService();
    const details = await service.updateCustomerVerification(
      userId,
      input,
